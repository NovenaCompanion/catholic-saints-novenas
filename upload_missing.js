...
async function uploadMissing() {
  const saintsData = JSON.parse(fs.readFileSync('missing_saints.json', 'utf8'));
  ...
  console.log('Missing saints upload completed!');
}

uploadMissing().catch(console.error).finally(() => process.exit(0));