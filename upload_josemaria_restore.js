...
async function restoreJosemaria() {
  const saintsData = JSON.parse(fs.readFileSync('restore_josemaria.json', 'utf8'));
  ...
  for (const saint of saintsData) {
    try {
      ...
      console.log(`Restored: ${saint.name} - ${saint.title} (ID: ${saintId})`);
      ...
      console.error(`Error restoring ${saint.name}:`, error);
    }
  }
  
  console.log('St. Josemaria Escriva entries restored!');
}

restoreJosemaria().catch(console.error).finally(() => process.exit(0));